...
###############################################################    
    ...
    def check_if_trained(self, kwargs):
        ...
        if response and response.status_code == 200 and len(response.json()["faces"]) > 0 and response.json()["faces"][0]["id"] == self.healthcheck_face_name:
                ...
                if len(faceNames) > 0 and faceNames[0] in self._getKnownFaces():
                    ...
                    for filename in result_dict_dict:
                        ...
                        if result_dict_dict[filename]["count"] == 1:
                            ...
                            # trigger teaching
                            self.teach_name_by_file(self.teach_url,result_dict_dict[filename]["faces"][0]["id"],new_filename)
                else:
                    #unknown face
                    self._processUnkownFaceFound(result_dict_dict)
        else:
